...
final saved = <String, String>{
  ...
  'pdsEndpoint': session.atprotoPdsEndpoint ?? '',
...
final restored = restoreOAuthSession(
  ...
  scope: saved['scope'],
  expiresAt: DateTime.parse(saved['expiresAt']!),
  sub: saved['sub'],
  clientId: saved['clientId'],
  pdsEndpoint: saved['pdsEndpoint'],
...
When the access token is a JWT, `restoreOAuthSession` can recover missing token
metadata from the token payload. For providers that issue opaque access tokens,
persist and restore `scope`, `expiresAt`, `sub`, and the PDS endpoint from the
