...
final contactDescriptor = XRPCObjectDescriptor<Contact>(
  ...
  defName: 'contact',
  fromJson: (json) =>
      const ContactConverter().fromJson(json.cast<String, dynamic>()),
  toJson: const ContactConverter().toJson,
  matches: Contact.validate,
);

final methodDescriptor =
    XRPCMethodDescriptor<EmptyData, EmptyData, ServerDescribeServerOutput>(
      nsid: NSID.parse('com.atproto.server.describeServer'),
      kind: XRPCMethodKind.query,
      outputFromJson: (json) => const ServerDescribeServerOutputConverter()
          .fromJson(json.cast<String, dynamic>()),
      outputToJson: const ServerDescribeServerOutputConverter().toJson,
      errors: const [],
    );

final comAtprotoServerDescribeServer = methodDescriptor;
