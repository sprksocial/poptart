...
If you want generated request and response types for Bluesky and AT Protocol
lexicons, add `poptart_lex` too:
...
- Call generated lexicon methods when `poptart_lex` is installed.
...
This example uses `poptart_lex` for the generated `getProfile` method and typed
input/output models.
...
Most apps can start with `poptart` and optionally `poptart_lex`. Add a focused
package later only when you know you want a smaller dependency or a lower-level
tool:

- Add `poptart_lex` for generated lexicon methods and models.
